...
#define MALLOC2(p, b, s, type) {if ((b) > 0) { \
                            p= (type)malloc(b); if (!(p)) { \
                            fprintf(stderr, "gpc malloc failure: %s\n", s); \
		            exit(0);}} else p= NULL;}
...
{
  NUL, EMX, ELI, TED, ERI, RED, IMM, IMN, EMN, EMM, LED, ILI, BED, IRI, IMX, FUL /* Full non-intersection */
} vertex_type;

typedef enum /* Horizontal edge states */
{
  NH, BH, TH /* Top horizontal edge */
} h_state;

typedef enum /* Edge bundle state */
{
  UNBUNDLED, BUNDLE_HEAD, BUNDLE_TAIL /* Passive bundle tail node */
} bundle_state;
...
static edge_node **bound_list(lmt_node **lmt, double y)
{
  ...
  if(! *lmt)
  {
    ...
    MALLOC2(*lmt, sizeof(lmt_node), "LMT insertion", lmt_node *);
    (*lmt)->y = y;
...
static void add_to_sbtree(int *entries, sb_tree **sbtree, double y)
{
  if(! *sbtree)
  {
    ...
    MALLOC2(*sbtree, sizeof(sb_tree), "scanbeam tree insertion", sb_tree *);
    (*sbtree)->y = y;
    (*sbtree)->less = NULL;
    (*sbtree)->more = NULL;
    (*entries)++;
...
static edge_node *build_lmt(lmt_node **lmt, sb_tree **sbtree, int *sbt_entries, gpc_polygon *p, int type, gpc_op op)
{
  ...
  MALLOC2(edge_table, total_vertices * sizeof(edge_node), "edge table creation", edge_node *);
...
static void add_intersection(it_node **it, edge_node *edge0, edge_node *edge1, double x, double y)
{
  ...
  if(! *it)
  {
    ...
    MALLOC2(*it, sizeof(it_node), "IT insertion", it_node *);
    (*it)->ie[0] = edge0;
    (*it)->ie[1] = edge1;
    (*it)->point.x = x;
    (*it)->point.y = y;
...
static void add_st_edge(st_node **st, it_node **it, edge_node *edge, double dy)
{
  ...
  if(! *st)
  {
    ...
    MALLOC2(*st, sizeof(st_node), "ST insertion", st_node *);
    (*st)->edge = edge;
    (*st)->xb = edge->xb;
    (*st)->xt = edge->xt;
    (*st)->dx = edge->dx;
...
static void add_left(polygon_node *p, double x, double y)
{
  ...
  MALLOC2(nv, sizeof(vertex_node), "vertex node creation", vertex_node *);
  nv->x = x;
  nv->y = y;
...
static void add_local_min(polygon_node **p, edge_node *edge, double x, double y)
{
  ...
  MALLOC2(*p, sizeof(polygon_node), "polygon node creation", polygon_node *);
...
static void add_vertex(vertex_node **t, double x, double y)
{
  if(!(*t))
  {
    ...
    MALLOC2(*t, sizeof(vertex_node), "tristrip vertex creation", vertex_node *);
    (*t)->x = x;
    (*t)->y = y;
    (*t)->next = NULL;
  }
...
static void new_tristrip(polygon_node **tn, edge_node *edge, double x, double y)
{
  if(!(*tn))
  {
    ...
    MALLOC2(*tn, sizeof(polygon_node), "tristrip node creation", polygon_node *);
    (*tn)->next = NULL;
    (*tn)->v[LEFT] = NULL;
    (*tn)->v[RIGHT] = NULL;
    (*tn)->active = 1;
...
static bbox *create_contour_bboxes(gpc_polygon *p)
{
  ...
  MALLOC2(box, p->num_contours * sizeof(bbox), "Bounding box creation", bbox *);
...
static void minimax_test(gpc_polygon *subj, gpc_polygon *clip, gpc_op op)
{
  ...
  MALLOC2(o_table, subj->num_contours *clip->num_contours * sizeof(int), "overlap table creation", int *);
...
/*
void gpc_read_polygon2(CPolygon *poly, gpc_polygon *p)
{
  int c, v;
  C3Point point;

  p->num_contours = 1; // poly->GetSize();

  MALLOC2(p->hole, p->num_contours * sizeof(int),"hole flag array creation",int *);
  MALLOC2(p->contour, p->num_contours * sizeof(gpc_vertex_list), "contour creation",gpc_vertex_list *);

  for (c= 0; c < p->num_contours; c++)
  {
	  p->contour[c].num_vertices = poly->GetSize();
		p->hole[c]= FALSE; // Assume all contours to be external

    MALLOC2(p->contour[c].vertex, p->contour[c].num_vertices
           * sizeof(gpc_vertex), "vertex creation",gpc_vertex *);

    for (v= 0; v < p->contour[c].num_vertices; v++)
		{
    point = poly[v];
		p->contour[c].vertex[v].x = point.x;
		p->contour[c].vertex[v].y = point.y;
		}
  }
}


void gpc_write_polygon2(CPolygon *poly, gpc_polygon *p)
{
  int c, v, size=sizeof(int);
  unsigned char *ptr;
  C3Point point;

  // Calcul de la taille et allocation

  size=0;
  for (c= 0; c < p->num_contours; c++)
  {
		size += p->contour[c].num_vertices;
  }
  ...
  for (c= 0; c < p->num_contours; c++)
  {
    for (v= 0; v < p->contour[c].num_vertices; v++)
		{
    point.x = p->contour[c].vertex[v].x;
    point.y = p->contour[c].vertex[v].y;
    poly->SetAt(size, point);
		}
  }
}
*/
...
void gpc_add_contour(gpc_polygon *p, gpc_vertex_list *new_contour, int hole)
{
  ...
  MALLOC2(extended_hole, (p->num_contours+1) * sizeof(int), "contour hole addition", int *);

  /* Create an extended contour array */
  //  MALLOC(extended_contour, (p->num_contours + 1)
  //         * sizeof(gpc_vertex_list), "contour addition", gpc_vertex_list);

  MALLOC2(extended_contour, (p->num_contours+1) * sizeof(gpc_vertex_list), "contour addition", gpc_vertex_list *);
  ...
  MALLOC2(extended_contour[c].vertex, new_contour->num_vertices * sizeof(gpc_vertex), "contour addition", gpc_vertex *);
...
void gpc_polygon_clip(gpc_op op, gpc_polygon *subj, gpc_polygon *clip, gpc_polygon *result)
{
  ...
  MALLOC2(sbt, sbt_entries * sizeof(double), "sbt creation", double *);
  ...
  if(result->num_contours > 0)
  {
    ...
    MALLOC2(result->hole, result->num_contours * sizeof(int), "hole flag table creation", int *);

    //    MALLOC(result->contour, result->num_contours
    //           * sizeof(gpc_vertex_list), "contour creation", gpc_vertex_list);

    MALLOC2(result->contour, result->num_contours * sizeof(gpc_vertex_list), "contour creation", gpc_vertex_list *);
    ...
    for(poly = out_poly; poly; poly = npoly)
    {
      ...
      if(poly->active)
      {
        ...
        MALLOC2(result->contour[c].vertex, result->contour[c].num_vertices * sizeof(gpc_vertex), "vertex creation", gpc_vertex *);
...
void gpc_tristrip_clip(gpc_op op, gpc_polygon *subj, gpc_polygon *clip, gpc_tristrip *result)
{
  ...
  if(result->num_strips > 0)
  {
    ...
    MALLOC2(result->strip, result->num_strips * sizeof(gpc_vertex_list), "tristrip list creation", gpc_vertex_list *);
    ...
    for(tn = tlist; tn; tn = tnn)
    {
      ...
      if(tn->active > 2)
      {
        ...
        MALLOC2(result->strip[s].vertex, tn->active * sizeof(gpc_vertex), "tristrip creation", gpc_vertex *);
        v = 0;
        if(INVERT_TRISTRIPS)
        {
          lt = tn->v[RIGHT];
