...
// DK 070328 (LWPOLYLINE) : Support des couches logiques
  mp_asStringLayer.CreateBuffer(1<<12); // Buffer de 4 Ko
// MODIF_KOCH 070328
...
// MODIF_KOCH 070328
// DK 070328 (LWPOLYLINE) : Support des couches logiques
  mp_asStringLayer.SetClassName (mp_oStrObjectName+"::mp_asStringLayer");
// MODIF_KOCH 070328
...
// MODIF_KOCH 070328
// DK 070328 (LWPOLYLINE) : Support des couches logiques
  + mp_asStringLayer.GetBufferFree()
// MODIF_KOCH 070328
  ;
...
// MODIF_KOCH 070328
// DK 070328 (LWPOLYLINE) : Support des couches logiques
  + mp_asStringLayer.GetBufferSize()
// MODIF_KOCH 070328
  ;
...
// MODIF_KOCH 070328
// DK 070328 (LWPOLYLINE) : Support des couches logiques
  mp_asStringLayer.RemoveStringAll();
// MODIF_KOCH 070328
...
// MODIF_KOCH 070328
// DK 070328 (LWPOLYLINE) : Support des couches logiques
  if(i_panStrLayerName != NULL)
  {
    mp_sTempoObject.nLayerName = mp_asStringLayer.GetStringIndex(i_panStrLayerName);
  }
  else{}
// MODIF_KOCH 070328
...
/*----------------------------------------------------------------------------*/
/*--- END FUNCTION HEADER ----------------------------------------------------*/
int CSigContainer::AddLayer
( const char* i_panStrLayerName
)
{
  return mp_asStringLayer.AddString((char*) i_panStrLayerName);
}
...
/*----------------------------------------------------------------------------*/
/*--- END FUNCTION HEADER ----------------------------------------------------*/
int CSigContainer::GetLayerCount
( void
)
{
  return mp_asStringLayer.GetStringNumber();
}
...
/*----------------------------------------------------------------------------*/
/*--- END FUNCTION HEADER ----------------------------------------------------*/
char* CSigContainer::GetLayerName
( int i_nLayerIndex
)
{
  return mp_asStringLayer.GetStringPointer(i_nLayerIndex);
}
...
/*----------------------------------------------------------------------------*/
/*--- END FUNCTION HEADER ----------------------------------------------------*/
int CSigContainer::GetLayerIndex
( const char* i_panStrLayerName
)
{
  return mp_asStringLayer.GetStringIndex(i_panStrLayerName);
}
...
/*----------------------------------------------------------------------------*/
/*--- END FUNCTION HEADER ----------------------------------------------------*/
bool CSigContainer::GetLayerStatus
( int  i_nLayerIndex
)
{
  if(mp_asStringLayer.GetStringPointer(i_nLayerIndex) != NULL)
  {
    return (mp_asStringLayer.GetStringData(i_nLayerIndex) == 0) ? true : false;
  }
  else
  {
    return true;
...
/*----------------------------------------------------------------------------*/
/*--- END FUNCTION HEADER ----------------------------------------------------*/
void CSigContainer::SetLayerStatus
( int  i_nLayerIndex
, bool i_bVisible  // = true
)
{
  if(mp_asStringLayer.GetStringPointer(i_nLayerIndex) != NULL)
  {
    mp_asStringLayer.SetStringData
    ( i_nLayerIndex
    , (i_bVisible == true) ? 0 : 1
    );
  }
...
// MODIF_KOCH 070328
// DK 070328 (LWPOLYLINE) : Support des couches logiques
       && ((l_psDstObject = AddObject(CreateObject(l_psSrcObject->eObjectType, l_psSrcObject->poPolygon->GetSize(), i_panStrBlockName, i_panStrLayerName))) != NULL)
// MODIF_KOCH 070328
    )
    {
      // On copie les points directement par le conteneur CSigPolygon
