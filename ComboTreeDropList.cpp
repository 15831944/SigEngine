...
HTREEITEM ComboTreeDropList::AddString ( LPCTSTR lpszString, HTREEITEM hParent /*= NULL*/)
{
//  HTREEITEM hRoot = hParent;
  ...
  HTREEITEM hEndNode = DropListAddItem (hParent, strTreeBranch);
...
// GetLastSibling2 - return last sibling of node
// hItem - Handle of the tree item
HTREEITEM ComboTreeDropList::GetLastSibling2( HTREEITEM hItem ) 
{
...
BOOL ComboTreeDropList::SetCheck( HTREEITEM hItem, CheckType nCheck )
{
  ...
  if (( nState == CHECKED) || ( (nCheck == REFRESH) && (nState == CHILD_CHECKED)))
  {  
  ...
  if (m_pCombo && bNotify && m_BlockNotify)
  {
...
{
  if((GetItemState( hItem, TVIS_STATEIMAGEMASK ) >> 12) != nState)
    m_ChangedItems.Add((UINT) hItem);
...
void      ComboTreeDropList::ClearChangedItems
( void
)
{
  m_ChangedItems.RemoveAll();
...
HTREEITEM ComboTreeDropList::GetFirstChangedItem
( void
)
{
  m_ChangedIndex = 0;
  if(m_ChangedIndex < m_ChangedItems.GetSize())
  {
    return (HTREEITEM) m_ChangedItems.GetAt(m_ChangedIndex);
  }
  else
  {
    return NULL;
...
HTREEITEM ComboTreeDropList::GetNextChangedItem
( void
)
{
  m_ChangedIndex += 1;
...
BOOL ComboTreeDropList::CheckOnTries
( CString& i_roStrTries
, HTREEITEM i_hItemFrom // = NULL
)
{
  HTREEITEM l_hItem = GetRootItem();
  CString   l_oStrItemText;
  ...
  {
    if(ItemHasChildren(l_hItem))
    {
      l_nResult
      = CheckOnTries
         ( i_roStrTries
         , GetChildItem(l_hItem)
         )
      ;
    }
    else
    {
      l_oStrItemText = GetItemText(l_hItem);

      if(i_roStrTries.Left(l_oStrItemText.GetLength()).CollateNoCase(l_oStrItemText.Left(i_roStrTries.GetLength())) == 0)
      {
        l_nResult = TRUE;
      }
      else{}
    ...
    l_hItem = GetNextSiblingItem(l_hItem);
  }

  return l_nResult;
}

int ComboTreeDropList::SetCheckOnTries
( CString&  i_roStrTries
, HTREEITEM i_hItemFrom // = NULL
)
{
  ...
  while(l_hItem != NULL)
  {
    if(ItemHasChildren(l_hItem))
    {
      ...
      l_nItemState = GetItemState(l_hItem, TVIS_STATEIMAGEMASK) >> 12;
      if(l_nItemState == UNCHECKED)
      { // If no child checked, collapse
        CollapseBranch(l_hItem);
      }
      else{}
    }
    else
    {
      ...
      if(i_roStrTries.Left(l_oStrItemText.GetLength()).CollateNoCase(l_oStrItemText.Left(i_roStrTries.GetLength())) == 0)
      { // Check
        if(l_nItemState != CHECKED)
        {
          SetCheck(l_hItem, CHECK);
          ExpandBranch(l_hItem);
        }
        else{}
...
void ComboTreeDropList::SetNotify
( bool i_bNotify
)
{
  m_BlockNotify = i_bNotify;
}
...
int ComboTreeDropList::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
  ...
  if (m_pCombo && m_pCombo->GetHasCheckboxes ())
  {
    BOOL bCreate = m_imageState.Create(IDB_COMBOTREECHECKS, 13, 1, RGB(255,255,255) );
    if (bCreate)
    {
    ...
    {
      TCHAR* errMsg = _T("Can't create tree check image list! Make sure bitmap with \"IDB_COMBOTREECHECKS\" (note quotes) was added as resource");
      MessageBox (errMsg);
      return -1;
    }
  }
...
void ComboTreeDropList::OnKillFocus(CWnd* pNewWnd) 
{
  CTreeCtrl::OnKillFocus(pNewWnd);
  
  if(m_pCombo)
  {
    m_pCombo->SendMessage(CBN_KILLFOCUS);
//    m_pCombo->SendParentComboMessage(CBN_KILLFOCUS);
    m_pCombo->OnCancel();
  }
}
