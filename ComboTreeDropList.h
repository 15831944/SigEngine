...
protected:
  ...
  BOOL      CheckOnTries(CString& i_roStrTries, HTREEITEM i_hItemFrom = NULL);
  int       SetCheckOnTries(CString& i_roStrTries, HTREEITEM i_hItemFrom = NULL);
  void      SetNotify(bool i_bNotify);

protected:
  ComboTree* m_pCombo;
  TCHAR       m_Delimiter;
  CUIntArray m_ChangedItems;
  int        m_ChangedIndex;
  bool       m_BlockNotify;
