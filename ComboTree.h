...
class EXPSIGENGINE ComboTree : public CWnd
{
...
//Tree specific extensions 
  ...
  HTREEITEM GetLastSibling2( HTREEITEM hItem ) ;
...
//Checked tree extensions  
  ...
  void      ClearChangedItems(void);
  HTREEITEM GetRootItem(void);
  HTREEITEM GetFirstChangedItem(void);
  HTREEITEM GetNextChangedItem(void);
  BOOL      CheckOnTries(CString& i_roStrTries);
  int       SetCheckOnTries(CString& i_roStrTries);
  void      SetNotify(bool i_bNotify);
  BOOL      ItemHasChildren(HTREEITEM i_hItem);
  BOOL      DeleteItem(HTREEITEM i_hItem);
  BOOL      DeleteAll(void);
  void      ValidateOnEnter(BOOL i_bValidate);
...
//CComboBox emulation functions
  ...
  HTREEITEM AddString ( LPCTSTR lpszString, HTREEITEM hParent = NULL);
