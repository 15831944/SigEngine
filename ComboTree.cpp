...
HTREEITEM ComboTree::GetLastSibling2( HTREEITEM hItem )
{
  return m_Tree.GetLastSibling2( hItem );
}
...
void ComboTree::ClearChangedItems
( void
)
{
  m_Tree.ClearChangedItems();
}

HTREEITEM ComboTree::GetRootItem
( void
)
{
...
HTREEITEM ComboTree::GetFirstChangedItem
( void
)
{
  return m_Tree.GetFirstChangedItem();
}

HTREEITEM ComboTree::GetNextChangedItem
( void
)
{
  return m_Tree.GetNextChangedItem();
}

BOOL ComboTree::CheckOnTries
( CString& i_roStrTries
)
{
  return m_Tree.CheckOnTries(i_roStrTries);
}

int ComboTree::SetCheckOnTries
( CString& i_roStrTries
)
{
  return m_Tree.SetCheckOnTries(i_roStrTries);
}

void ComboTree::SetNotify
( bool i_bNotify
)
{
  m_Tree.SetNotify(i_bNotify);
}

BOOL ComboTree::ItemHasChildren
( HTREEITEM i_hItem
)
{
  return m_Tree.ItemHasChildren(i_hItem);
}

BOOL ComboTree::DeleteItem
( HTREEITEM i_hItem
)
{
  return m_Tree.DeleteItem(i_hItem);
}

BOOL ComboTree::DeleteAll(void)
{
  m_Edit.SetWindowText("");
  return m_Tree.DeleteAllItems();
}

void ComboTree::ValidateOnEnter
( BOOL i_bValidate
)
{
  m_Edit.ValidateOnEnter(i_bValidate);
}
...
/*
    if (! m_Tree.CreateEx ( WS_EX_LEFT | WS_EX_LTRREADING | WS_EX_RIGHTSCROLLBAR | WS_EX_TOOLWINDOW,
              ...
              treeRect, pDesktop, IDC_COMBOTREE_DROPDOWN, NULL))
*/
...
HTREEITEM ComboTree::AddString ( LPCTSTR lpszString, HTREEITEM hParent /*= NULL*/)
{
  return m_Tree.AddString (lpszString, hParent);
}
...
/*
    ...
    m_Edit.GetWindowText(l_oStrText);
    ...
    m_Edit.SetWindowText(l_oStrText);

    UpdateData(FALSE);

    m_Edit.SetSel(l_oStrText.GetLength(), l_oStrText.GetLength());
*/
  }
...
void ComboTree::OnSize(UINT nType, int cx, int cy) 
{
...
//  CRect l_oRectCombo;
  ...
  m_Edit.GetWindowRect(l_oRectEdit);
  m_Edit.GetParent()->ScreenToClient(l_oRectEdit);

  m_DropDownButton.GetWindowRect(l_oRectButton);
  m_DropDownButton.GetParent()->ScreenToClient(l_oRectButton);

//  m_Tree.GetWindowRect(l_oRectCombo);
//  m_Tree.GetParent()->ScreenToClient(l_oRectCombo);
  
  l_oRectEdit.right   = cx - l_oRectButton.Width();
//  l_oRectCombo.right  = cx + l_oRectButton.Width();

  l_oRectButton.left  = cx - l_oRectButton.Width();
  l_oRectButton.right = cx;
	
  m_Edit.MoveWindow(l_oRectEdit);
  m_DropDownButton.MoveWindow(l_oRectButton);
//  m_Tree.MoveWindow(l_oRectCombo);
  SetDroppedWidth(cx + 2);
}
