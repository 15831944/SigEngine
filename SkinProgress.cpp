...
// C..... Set the progress bar values
    ...
    ASSERT(i_nUpper > 0); // mp_nLower is 0, mp_nUpper CANNOT BE BEFORE mp_nLower
    mp_nUpper = i_nUpper;
...
#ifdef dCSP_SLIDERBAR_METHOD
    if
    ( // In case of slider display, the closeness of the RIGHT element from the LEFT can cause an over-write of the LEFT element with BAR and/or CENTER
         (mp_nPrevLeftAff != mp_nLeftAff)
      || (
             mp_nRightAff
...
// CDJ... Draw the right element of the bar
    if
    ( // In case of slider display, the closeness of the LEFT element from the RIGHT can cause an over-write of the RIGHT element with BAR and/or CENTER
         (mp_nPrevRightAff != mp_nRightAff)
#ifdef dCSP_SLIDERBAR_METHOD
      || (
              mp_nLeftAff
...
// D..... Display the progress bar
  ...
  BITMAP   l_sStretchBmp;

  l_oDcStretch.CreateCompatibleDC(&l_oDC);
  l_poOldStretch = l_oDcStretch.SelectObject(mp_poStretchBmp);

  mp_poStretchBmp->GetBitmap(&sStretchBmp);
  l_oDC.BitBlt(0, 0, l_sStretchBmp.bmWidth, l_sStretchBmp.bmHeight, &l_oDcStretch, 0, 0, SRCCOPY);
